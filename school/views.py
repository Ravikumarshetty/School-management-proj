...
def admin_attendance_view(request):
    ...
    #
    #         'Todays Attendance ',
    #         'Today:- you are not attend the class ',
    #         'karnalax987@gmail.com',
    #         ['shreyasj077@gmail.com'],
    #         fail_silently=False,
    #
    # )
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            for i in range(len(Attendances)):
                ...
                # AttendanceModel.fee=fee
                AttendanceModel.date=date
                AttendanceModel.present_status=Attendances[i]
                AttendanceModel.roll=students[i].roll
                AttendanceModel.fee = students[i].fee
...
def student_attendance_view(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            attendancedata=models.Attendance.objects.all().filter(date=date,cl=studentdata[0].cl,roll=studentdata[0].fee)
            # studentdata2 = models.StudentExtra.objects.all().filter(mail=request.fee.id, status=True)
            mylist=zip(attendancedata,studentdata)
